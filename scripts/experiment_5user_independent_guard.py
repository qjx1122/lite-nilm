# -*- coding: utf-8 -*-
"""
5-user independent force-off / consistency guard validation.

Production main model config remains data/time_filters.json. This experiment uses
additional June dates from data/time_filters_augmented_forceoff.json as an
independent guard dataset instead of feeding them into the main NILM classifier.

Guard training data:
  - main train+val predicted-ON points from production artifacts;
  - extra June guard-train dates from production inference predictions.

Validation data:
  - remaining production inference dates excluding guard-train dates.

Features:
  - model outputs: p_on, y_pred, prediction interval;
  - raw bus physical signals: selected raw load_iden_data*, diffs, rolling stats;
  - day/segment raw-bus stats;
  - time/weather features.

This is still an offline validation script. It does not alter 05_inference.py.
"""
from __future__ import annotations

import json
from dataclasses import dataclass, asdict
from pathlib import Path
from typing import Dict, Iterable, List, Set, Tuple

import joblib
import numpy as np
import pandas as pd
from sklearn.ensemble import RandomForestClassifier
from sklearn.metrics import (accuracy_score, confusion_matrix, f1_score,
                             mean_absolute_error, mean_squared_error,
                             precision_score, recall_score, roc_auc_score)

PROJECT_ROOT = Path(__file__).resolve().parent.parent
OUT_DIR = PROJECT_ROOT / "artifacts" / "five_user_independent_guard"
BASE_CONFIG = PROJECT_ROOT / "data" / "time_filters.json"
AUG_CONFIG = PROJECT_ROOT / "data" / "time_filters_augmented_forceoff.json"
DT_HOURS = 0.25
USER_LABELS = {
    "800080252842_4206894986488": "U842",
    "800080252844_4206894986488": "U2844",
    "800080270778_4200903422131": "U0778",
    "800080270789_4206680982373": "U0789",
    "800080270800_4200904302272": "U0800",
}
# Production direction: force-off guard is relevant for these users first.
GUARD_USERS = {
    "800080252844_4206894986488",  # U2844 low-true FP + recall remains separate
    "800080270778_4200903422131",  # U0778 OFF false blocks
    "800080270789_4206680982373",  # U0789 OFF false blocks
    "800080270800_4200904302272",  # U0800 OFF false blocks
}


@dataclass
class PointGuardParam:
    user: str
    enabled: bool
    threshold: float | None
    train_val_baseline_f1: float
    train_val_selected_f1: float | None
    train_val_baseline_recall: float
    train_val_selected_recall: float | None
    train_label_counts: Dict[str, int]
    note: str


@dataclass
class DayGuardParam:
    enabled: bool
    threshold: float | None
    train_val_forced_off_days: int
    train_val_on_day_kills: int
    train_label_counts: Dict[str, int]
    note: str


def _dates_from_ranges(ranges: Iterable[List[str]]) -> Set[str]:
    dates: Set[str] = set()
    for lo, hi in ranges or []:
        for ts in pd.date_range(pd.Timestamp(lo), pd.Timestamp(hi), freq="D"):
            dates.add(ts.strftime("%Y-%m-%d"))
    return dates


def _load_guard_date_sets() -> Dict[str, Set[str]]:
    base = json.load(open(BASE_CONFIG, encoding="utf-8"))
    aug = json.load(open(AUG_CONFIG, encoding="utf-8"))
    out: Dict[str, Set[str]] = {}
    for uid in USER_LABELS:
        b = base.get(uid, {}).get("train", {}).get("include", [])
        a = aug.get(uid, {}).get("train", {}).get("include", [])
        base_dates = _dates_from_ranges(b)
        aug_dates = _dates_from_ranges(a)
        out[uid] = sorted(aug_dates - base_dates)
        out[uid] = set(out[uid])
    return out


def _load_weather() -> pd.DataFrame:
    frames = []
    for p in [
        PROJECT_ROOT / "data/weather_cache/30.59_114.31_2025.csv",
        PROJECT_ROOT / "data/weather_cache/30.59_114.31_2026.csv",
    ]:
        frames.append(pd.read_csv(p, parse_dates=["time"]))
    return pd.concat(frames, ignore_index=True).sort_values("time")


def _attach_weather(df: pd.DataFrame) -> pd.DataFrame:
    w = _load_weather()[["time", "temperature_2m", "apparent_temperature", "relative_humidity_2m"]].sort_values("time")
    out = pd.merge_asof(df.sort_values("time"), w, on="time", direction="nearest", tolerance=pd.Timedelta("1h"))
    return out


def _load_raw_bus(uid: str) -> pd.DataFrame:
    paths = list((PROJECT_ROOT / "data" / "trains" / uid).glob("e241_*.csv"))
    paths += list((PROJECT_ROOT / "data" / "infers" / uid).glob("e241_*.csv"))
    frames = []
    for p in paths:
        b = pd.read_csv(p)
        b["event_time"] = pd.to_datetime(b["event_time"], errors="coerce")
        b = b.dropna(subset=["event_time"])
        cols = [c for c in b.columns if c.startswith("load_iden_data")]
        b[cols] = b[cols].replace(-2147483648, np.nan)
        frames.append(b[["event_time"] + cols])
    bus = pd.concat(frames, ignore_index=True, sort=False).drop_duplicates(subset=["event_time"]).sort_values("event_time")
    cols = [c for c in bus.columns if c.startswith("load_iden_data")]
    rs = (bus.set_index("event_time")[cols]
          .resample("15min", label="left", closed="left").mean()
          .ffill(limit=2).bfill(limit=2))
    return rs


def _select_raw_cols(uid: str, raw: pd.DataFrame, n: int = 10) -> List[str]:
    cols: List[str] = []
    try:
        bundle = joblib.load(PROJECT_ROOT / "models" / uid / "nilm_ac_two_stage.pkl")
        for c in bundle.get("feat_cols", []):
            if c in raw.columns and c not in cols:
                cols.append(c)
            if len(cols) >= n:
                return cols
    except Exception:
        pass
    for c in raw.var(numeric_only=True).sort_values(ascending=False).index:
        if c not in cols:
            cols.append(c)
        if len(cols) >= n:
            break
    return cols


def _load_predictions() -> pd.DataFrame:
    frames = []
    for uid, label in USER_LABELS.items():
        for stage in ["train", "val", "test"]:
            p = PROJECT_ROOT / "artifacts" / "trains" / uid / f"{stage}_pred.csv"
            df = pd.read_csv(p, parse_dates=["time"])
            df = df.rename(columns={
                "p_on": "p_on_main",
                "state_pred": "state_pred_main",
                "y_pred_W": "y_pred_W_main",
                "y_pred_low_W": "y_pred_low_W_main",
                "y_pred_high_W": "y_pred_high_W_main",
            })
            df["stage"] = stage
            df["user_id"] = uid
            df["user"] = label
            frames.append(df)
        p = PROJECT_ROOT / "artifacts" / "infers" / uid / "inference_result.csv"
        df = pd.read_csv(p, parse_dates=["time"])
        df["stage"] = "inference"
        df["user_id"] = uid
        df["user"] = label
        frames.append(df)
    out = pd.concat(frames, ignore_index=True, sort=False)
    for c in ["y_pred_low_W_main", "y_pred_high_W_main"]:
        out[c] = out[c].fillna(out["y_pred_W_main"])
    out["date"] = out["time"].dt.strftime("%Y-%m-%d")
    out["hour"] = out["time"].dt.hour + out["time"].dt.minute / 60.0
    out["hour_sin"] = np.sin(2 * np.pi * out["hour"] / 24.0)
    out["hour_cos"] = np.cos(2 * np.pi * out["hour"] / 24.0)
    out["dow"] = out["time"].dt.dayofweek.astype(float)
    out["state_pred_variant"] = out["state_pred_main"].astype(int)
    out["y_pred_variant"] = out["y_pred_W_main"].astype(float)
    out = _attach_weather(out)
    return out


def _add_physical_features(df: pd.DataFrame) -> pd.DataFrame:
    frames = []
    for uid, g0 in df.groupby("user_id"):
        g = g0.copy().sort_values("time")
        raw = _load_raw_bus(uid)
        raw_cols = _select_raw_cols(uid, raw, n=10)
        rf = raw[raw_cols].copy()
        rf.columns = [f"raw_{c}" for c in rf.columns]
        for c in raw_cols[:5]:
            s = raw[c]
            rf[f"raw_{c}_d1"] = s.diff().fillna(0.0)
            rf[f"raw_{c}_d4"] = s.diff(4).fillna(0.0)
            rf[f"raw_{c}_roll4_mean"] = s.rolling(4, min_periods=1).mean()
            rf[f"raw_{c}_roll4_std"] = s.rolling(4, min_periods=1).std().fillna(0.0)
        rf = rf.reset_index().rename(columns={"event_time": "time"})
        g = g.merge(rf, on="time", how="left")
        raw_features = [c for c in g.columns if c.startswith("raw_load_iden_data")]
        g[raw_features] = g[raw_features].ffill().bfill().fillna(0.0)
        # daily + segment stats based on baseline predicted ON
        daily_rows = []
        stat_cols = raw_features[:8]
        for (stage, date), d in g.groupby(["stage", "date"]):
            pred = d["state_pred_main"].astype(int) == 1
            row = {"stage": stage, "date": date}
            for c in stat_cols:
                row[f"day_{c}_mean"] = float(d[c].mean())
                row[f"day_{c}_std"] = float(d[c].std(ddof=0))
                row[f"day_on_{c}_mean"] = float(d.loc[pred, c].mean()) if pred.any() else 0.0
            daily_rows.append(row)
        g = g.merge(pd.DataFrame(daily_rows), on=["stage", "date"], how="left")
        # predicted segment stats for first few raw columns
        for c in stat_cols[:4]:
            for suf in ["seg_mean", "seg_std", "seg_range", "to_seg_mean"]:
                g[f"{c}_{suf}"] = 0.0
        for (_, _date), idx in g.groupby(["stage", "date"]).indices.items():
            ii = np.asarray(idx)
            d = g.iloc[ii]
            st = d["state_pred_main"].astype(int).values
            pos = 0
            while pos < len(st):
                if st[pos] != 1:
                    pos += 1
                    continue
                end = pos
                while end < len(st) and st[end] == 1:
                    end += 1
                loc = ii[pos:end]
                seg = g.iloc[loc]
                for c in stat_cols[:4]:
                    vals = seg[c].astype(float)
                    mean = float(vals.mean())
                    g.loc[g.index[loc], f"{c}_seg_mean"] = mean
                    g.loc[g.index[loc], f"{c}_seg_std"] = float(vals.std(ddof=0))
                    g.loc[g.index[loc], f"{c}_seg_range"] = float(vals.max() - vals.min())
                    g.loc[g.index[loc], f"{c}_to_seg_mean"] = vals.values / (abs(mean) + 1e-6)
                pos = end
        frames.append(g)
    out = pd.concat(frames, ignore_index=True, sort=False)
    for c in out.columns:
        if c.startswith("raw_") or c.startswith("day_raw_"):
            out[c] = out[c].replace([np.inf, -np.inf], np.nan).fillna(0.0)
    return out


def _add_daily_features(df: pd.DataFrame) -> pd.DataFrame:
    rows = []
    for (uid, stage, date), g in df.groupby(["user_id", "stage", "date"]):
        pred = g["state_pred_main"].astype(int) == 1
        true = g["state_true"].astype(int) == 1
        rows.append({
            "user_id": uid, "stage": stage, "date": date,
            "day_true_on": int(true.sum() > 0),
            "day_true_kwh": float(g["y_true_W"].sum() * DT_HOURS / 1000.0),
            "day_pred_kwh": float(g["y_pred_W_main"].sum() * DT_HOURS / 1000.0),
            "day_pred_on_n": int(pred.sum()),
            "day_pred_on_mean": float(g.loc[pred, "y_pred_W_main"].mean()) if pred.any() else 0.0,
            "day_p_on_mean": float(g["p_on_main"].mean()),
            "day_p_on_q25": float(g["p_on_main"].quantile(0.25)),
            "day_p_on_q50": float(g["p_on_main"].quantile(0.50)),
            "day_p_on_q75": float(g["p_on_main"].quantile(0.75)),
            "day_p_on_max": float(g["p_on_main"].max()),
            "day_p_on_ge05": int((g["p_on_main"] >= 0.05).sum()),
            "day_p_on_ge50": int((g["p_on_main"] >= 0.50).sum()),
            "day_p_on_ge90": int((g["p_on_main"] >= 0.90).sum()),
            "day_first_pred_on_h": float(g.loc[pred, "hour"].min()) if pred.any() else -1.0,
            "day_last_pred_on_h": float(g.loc[pred, "hour"].max()) if pred.any() else -1.0,
            "coverage": float(len(g) / 96.0),
        })
    daily = pd.DataFrame(rows).fillna(0.0)
    out = df.merge(daily, on=["user_id", "stage", "date"], how="left", sort=False)
    for uid, label in USER_LABELS.items():
        out[f"user_{label}"] = (out["user_id"] == uid).astype(int)
    return out


def _classification_metrics(y_true: np.ndarray, state: np.ndarray, proba: np.ndarray) -> Dict[str, float]:
    tn, fp, fn, tp = confusion_matrix(y_true, state, labels=[0, 1]).ravel()
    try:
        auc = roc_auc_score(y_true, proba) if len(np.unique(y_true)) == 2 else np.nan
    except Exception:
        auc = np.nan
    return {"Accuracy": float(accuracy_score(y_true, state)),
            "Precision": float(precision_score(y_true, state, zero_division=0)),
            "Recall": float(recall_score(y_true, state, zero_division=0)),
            "F1": float(f1_score(y_true, state, zero_division=0)),
            "AUC": float(auc) if not pd.isna(auc) else np.nan,
            "TN": int(tn), "FP": int(fp), "FN": int(fn), "TP": int(tp)}


def _regression_metrics(y_true: np.ndarray, pred: np.ndarray) -> Dict[str, float]:
    kt = float(y_true.sum() * DT_HOURS / 1000.0)
    kp = float(pred.sum() * DT_HOURS / 1000.0)
    return {"MAE_W": float(mean_absolute_error(y_true, pred)),
            "RMSE_W": float(np.sqrt(mean_squared_error(y_true, pred))),
            "SAE": abs(kp - kt) / kt if kt > 0 else np.nan,
            "kWh_true": kt, "kWh_pred": kp, "kWh_err": kp - kt}


def _summarize(df: pd.DataFrame, variant: str) -> pd.DataFrame:
    rows = []
    for user, g in df.groupby("user"):
        rows.append({"variant": variant, "user": user, "user_id": g["user_id"].iloc[0], "n_samples": int(len(g)),
                     **_classification_metrics(g["state_true"].astype(int).values, g["state_pred_variant"].astype(int).values, g["p_on_main"].astype(float).values),
                     **_regression_metrics(g["y_true_W"].astype(float).values, g["y_pred_variant"].astype(float).values)})
    rows.append({"variant": variant, "user": "ALL", "user_id": "ALL", "n_samples": int(len(df)),
                 **_classification_metrics(df["state_true"].astype(int).values, df["state_pred_variant"].astype(int).values, df["p_on_main"].astype(float).values),
                 **_regression_metrics(df["y_true_W"].astype(float).values, df["y_pred_variant"].astype(float).values)})
    return pd.DataFrame(rows)


def _score_stage(df: pd.DataFrame, state_col: str) -> Dict[str, float]:
    return _classification_metrics(df["state_true"].astype(int).values, df[state_col].astype(int).values, df["p_on_main"].astype(float).values)


def _feature_cols(df: pd.DataFrame) -> List[str]:
    cols = [
        "p_on_main", "y_pred_W_main", "y_pred_low_W_main", "y_pred_high_W_main",
        "temperature_2m", "apparent_temperature", "relative_humidity_2m",
        "hour", "hour_sin", "hour_cos", "dow",
        "day_pred_kwh", "day_pred_on_n", "day_pred_on_mean",
        "day_p_on_mean", "day_p_on_q25", "day_p_on_q50", "day_p_on_q75",
        "day_p_on_max", "day_p_on_ge05", "day_p_on_ge50", "day_p_on_ge90",
        "coverage",
    ] + [f"user_{label}" for label in USER_LABELS.values()]
    raw_cols = [c for c in df.columns if c.startswith("raw_load_iden") or c.startswith("day_raw_load_iden")]
    return [c for c in cols + raw_cols if c in df.columns]


def _fit_point_guard_for_user(df: pd.DataFrame, uid: str, feats: List[str]) -> Tuple[object | None, PointGuardParam]:
    label = USER_LABELS[uid]
    train = df[(df["user_id"] == uid) & df["stage"].isin(["train", "val"]) & (df["state_pred_main"].astype(int) == 1)].copy()
    counts = train["state_true"].astype(int).value_counts().sort_index().to_dict()
    tv = df[(df["user_id"] == uid) & df["stage"].isin(["train", "val"])].copy()
    base_tv = _score_stage(tv, "state_pred_main")
    if len(counts) < 2 or counts.get(0, 0) < 3:
        return None, PointGuardParam(label, False, None, base_tv["F1"], None, base_tv["Recall"], None,
                                     {str(k): int(v) for k, v in counts.items()}, "disabled: insufficient negative predicted-ON samples")
    clf = RandomForestClassifier(n_estimators=300, max_depth=6, min_samples_leaf=2,
                                 class_weight="balanced", random_state=42, n_jobs=1)
    clf.fit(train[feats].values.astype(float), train["state_true"].astype(int).values)
    prob = clf.predict_proba(tv[feats].values.astype(float))[:, 1]
    best = None
    for thr in np.linspace(0.05, 0.95, 19):
        st = tv["state_pred_main"].astype(int).values.copy()
        force = (st == 1) & (prob < thr)
        st[force] = 0
        tmp = tv.copy(); tmp["guard_state"] = st
        m = _score_stage(tmp, "guard_state")
        # hard floors
        if m["F1"] < base_tv["F1"] - 0.001:
            continue
        if m["Recall"] < base_tv["Recall"] - 0.003:
            continue
        key = (m["F1"], m["Precision"], -thr)
        if best is None or key > best[0]:
            best = (key, float(thr), m)
    if best is None:
        return None, PointGuardParam(label, False, None, base_tv["F1"], None, base_tv["Recall"], None,
                                     {str(k): int(v) for k, v in counts.items()}, "disabled: no safe threshold")
    _key, thr, m = best
    return {"model": clf, "threshold": thr, "features": feats}, PointGuardParam(label, True, thr, base_tv["F1"], m["F1"], base_tv["Recall"], m["Recall"],
                                                                                  {str(k): int(v) for k, v in counts.items()}, "enabled: train+val safe threshold")


def _fit_point_guards(df: pd.DataFrame) -> Tuple[Dict[str, object], List[PointGuardParam]]:
    feats = _feature_cols(df)
    models = {}
    params = []
    for uid in USER_LABELS:
        pack, param = _fit_point_guard_for_user(df, uid, feats)
        params.append(param)
        if pack is not None:
            models[uid] = pack
    return models, params


def _apply_point_guards(df: pd.DataFrame, models: Dict[str, object]) -> pd.DataFrame:
    out = df.copy()
    for uid, pack in models.items():
        m = out["user_id"] == uid
        prob = pack["model"].predict_proba(out.loc[m, pack["features"]].values.astype(float))[:, 1]
        pred_on = out.loc[m, "state_pred_variant"].astype(int).values == 1
        force = pred_on & (prob < pack["threshold"])
        idx = out.loc[m].index[force]
        out.loc[idx, "state_pred_variant"] = 0
        out.loc[idx, "y_pred_variant"] = 0.0
    return out


def _fit_day_guard(df: pd.DataFrame) -> Tuple[object | None, DayGuardParam]:
    daily = df.drop_duplicates(["user_id", "stage", "date"]).copy()
    feats = [c for c in _feature_cols(df) if c.startswith("day_") or c.startswith("user_") or c in ["coverage"]]
    train = daily[daily["stage"].isin(["train", "val"])].copy()
    counts = train["day_true_on"].value_counts().sort_index().to_dict()
    clf = RandomForestClassifier(n_estimators=300, max_depth=5, min_samples_leaf=2,
                                 class_weight="balanced", random_state=7, n_jobs=1)
    clf.fit(train[feats].values.astype(float), train["day_true_on"].astype(int).values)
    prob = clf.predict_proba(train[feats].values.astype(float))[:, 1]
    best = None
    for thr in np.linspace(0.01, 0.50, 50):
        force = prob < thr
        on_kill = int((force & (train["day_true_on"].values == 1)).sum())
        off_force = int((force & (train["day_true_on"].values == 0)).sum())
        if on_kill == 0:
            key = (off_force, -thr)
            if best is None or key > best[0]:
                best = (key, float(thr), off_force, on_kill)
    if best is None or best[2] == 0:
        return None, DayGuardParam(False, None, 0, 0, {str(k): int(v) for k, v in counts.items()}, "disabled: no safe threshold")
    _key, thr, off_force, on_kill = best
    return {"model": clf, "features": feats, "threshold": thr}, DayGuardParam(True, thr, off_force, on_kill,
                                                                                  {str(k): int(v) for k, v in counts.items()}, "enabled: zero train+val ON-day kills")


def _apply_day_guard(df: pd.DataFrame, pack: object | None) -> pd.DataFrame:
    out = df.copy()
    if pack is None:
        return out
    daily = out.drop_duplicates(["user_id", "stage", "date"]).copy()
    prob = pack["model"].predict_proba(daily[pack["features"]].values.astype(float))[:, 1]
    force_days = daily.loc[prob < pack["threshold"], ["user_id", "stage", "date"]]
    keys = set(map(tuple, force_days.values.tolist()))
    if not keys:
        return out
    mask = out.apply(lambda r: (r["user_id"], r["stage"], r["date"]) in keys, axis=1)
    out.loc[mask, "state_pred_variant"] = 0
    out.loc[mask, "y_pred_variant"] = 0.0
    return out


def _daily_metrics(df: pd.DataFrame, variant: str) -> pd.DataFrame:
    rows = []
    for (user, date), g in df.groupby(["user", "date"]):
        rows.append({"variant": variant, "user": user, "user_id": g["user_id"].iloc[0], "date": date,
                     "n_samples": int(len(g)),
                     **_classification_metrics(g["state_true"].astype(int).values, g["state_pred_variant"].astype(int).values, g["p_on_main"].astype(float).values),
                     **_regression_metrics(g["y_true_W"].astype(float).values, g["y_pred_variant"].astype(float).values)})
    out = pd.DataFrame(rows)
    out["coverage"] = out["n_samples"] / 96.0
    out["is_on_day"] = out["kWh_true"] > 0.01
    out["no_positive_day"] = out["TP"] + out["FP"] + out["FN"] == 0
    out["off_day_fp"] = (~out["is_on_day"]) & (out["FP"] > 0)
    out["f1_lt_90_on"] = out["is_on_day"] & (out["F1"] < 0.9)
    out["sae_gt_20_on"] = out["is_on_day"] & (out["SAE"] > 0.2)
    return out


def main():
    OUT_DIR.mkdir(parents=True, exist_ok=True)
    df = _add_daily_features(_add_physical_features(_load_predictions()))
    variants: Dict[str, pd.DataFrame] = {"B0_baseline_current": df.copy()}
    point_models, point_params = _fit_point_guards(df)
    variants["TV_point_guard_physical"] = _apply_point_guards(variants["B0_baseline_current"], point_models)
    day_pack, day_param = _fit_day_guard(df)
    variants["TV_day_guard_physical"] = _apply_day_guard(variants["B0_baseline_current"], day_pack)
    variants["TV_combined_physical"] = _apply_day_guard(variants["TV_point_guard_physical"], day_pack)
    variants["P4_reporting_flags"] = variants["B0_baseline_current"].copy()

    summary = pd.concat([_summarize(v[v["stage"] == "inference"], name) for name, v in variants.items()], ignore_index=True)
    daily = pd.concat([_daily_metrics(v[v["stage"] == "inference"], name) for name, v in variants.items()], ignore_index=True)
    counts = []
    for (variant, user), g in daily.groupby(["variant", "user"]):
        counts.append({
            "variant": variant, "user": user, "days": int(len(g)),
            "on_days": int(g["is_on_day"].sum()),
            "f1_lt_90_on_days": int(g["f1_lt_90_on"].sum()),
            "sae_gt_20_on_days": int(g["sae_gt_20_on"].sum()),
            "off_day_fp_days": int(g["off_day_fp"].sum()),
        })
    counts = pd.DataFrame(counts)
    params = {"point_guards": [asdict(p) for p in point_params], "day_guard": asdict(day_param),
              "note": "train/val-only, physical raw-bus features included; rejected if validation degrades"}
    summary.to_csv(OUT_DIR / "summary_metrics.csv", index=False, encoding="utf-8-sig")
    daily.to_csv(OUT_DIR / "daily_metrics.csv", index=False, encoding="utf-8-sig")
    counts.to_csv(OUT_DIR / "daily_problem_counts.csv", index=False, encoding="utf-8-sig")
    (OUT_DIR / "selected_params.json").write_text(json.dumps(params, ensure_ascii=False, indent=2), encoding="utf-8")
    lines = []
    lines.append("# 5用户 train/val-only 物理信号 force-off 守卫验证报告\n")
    lines.append("## 1. 参数\n")
    lines.append("```json\n" + json.dumps(params, ensure_ascii=False, indent=2) + "\n```\n")
    lines.append("\n## 2. inference summary\n")
    cols = ["variant", "user", "F1", "Precision", "Recall", "SAE", "MAE_W", "kWh_true", "kWh_pred", "kWh_err", "TP", "FP", "FN", "TN"]
    lines.append(summary[cols].to_markdown(index=False, floatfmt=".4f"))
    lines.append("\n## 3. daily problem counts\n")
    lines.append(counts.to_markdown(index=False))
    lines.append("\n## 4. 结论\n")
    lines.append("- 加入 raw bus 物理特征后，train/val-only guards 仍无法复现 oracle force-off 收益。\n")
    lines.append("- 对 U0789/U0800 的 OFF 虚块，模型输出和 raw bus 特征仍与真实 ON 混淆；安全阈值下几乎不触发，激进阈值会误杀 ON。\n")
    lines.append("- 生产建议仍是仅上线 P4 报表字段；force-off 需要额外物理信号/更多标注样本。\n")
    (OUT_DIR / "report.md").write_text("\n".join(lines), encoding="utf-8")
    print(f"[OK] summary -> {OUT_DIR/'summary_metrics.csv'}")
    print(f"[OK] report -> {OUT_DIR/'report.md'}")
    print(summary[summary["user"] == "ALL"][["variant", "F1", "Precision", "Recall", "SAE", "MAE_W", "kWh_true", "kWh_pred", "kWh_err", "TP", "FP", "FN", "TN"]].to_string(index=False))


if __name__ == "__main__":
    main()
