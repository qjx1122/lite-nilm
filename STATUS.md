...
- 已完成（2026-08-04 session 21）：**按推理落地顺序验证 6/19 低功率长时锚点/cap** → `scripts/experiment_u842_0619_lowpower_anchor.py` + `U842_20260619_LOWPOWER_ANCHOR_VALIDATION_REPORT.md`；P0 oracle target 上限：6/19 SAE 266.6%→0.56%（anchor=207W），6/27 SAE 42.0%→21.6%（anchor=308W），证明锚点方向有效；P1 锚点来自 train+val 低功率长时 ON（6/4/6/10/6/11）；P2 label-free feature_rule 全风险日验证失败，会误伤 train/test 高功率日，train_val SAE 0.16%→8.82%、test SAE 2.97%→15.03%。结论：功率锚有效，但风险识别器不合格，不能上线简单规则；下一步需更强 day-level low_power_long_improves classifier/补样本。
- 已完成（2026-08-04 session 22）：**按 6.2 增强特征训练/验证低功率长时风险识别器** → `scripts/experiment_u842_lowpower_risk_classifier.py` + `U842_LOWPOWER_RISK_CLASSIFIER_REPORT.md`；特征含 rawbus-to-current、raw73_on mean/std、早中晚 raw73/p_on、base/current_on_mean、p_on分位、cluster距离等；train+val 标签 `true_on_h>=10h & true_on_mean<400W`，正样本仅3天(6/4,6/10,6/11)。RF/ExtraTrees/LogReg 均能拟合 train 3/3，但 inference 只命中 6/8、6/9，漏掉目标 6/19、6/27（recall=0.4, precision=1.0）；应用锚点后 inference SAE 0.735%→1.707% 退化。Oracle 仍证明锚点可修 6/19/6/27，但识别器不合格。结论：增强特征仍不足，缺少“raw bus 高+baseline 高+true低功率”的同型样本/非空调负荷分解特征，不能上线。
- 已完成（2026-08-04 session 23）：**当前版本 5 用户全量 force-retrain 批量验证测试与逐用户问题指标分析** → `FIVE_USERS_CURRENT_BATCH_VALIDATION_REPORT.md`；命令 `run_batch_users.py --force-retrain --time-filter-config data/time_filters.json`，日志 `batch_run_20260804_060918.log`，5/5 `[OK]`，LightGBM active。池化 inference：F1=0.8495、P=0.7743、R=0.9408、SAE=10.08%、kWh 1113.1→1225.3。用户 inference：U842 F1=0.914/SAE=14.6%（主路径仍未并入P0-P3）；U2844 F1=0.895/SAE=11.2%；U0778 F1=0.946/SAE=0.38%但日级FP/SAE多；U0789 F1=0.799/SAE=26.6%且FP=742；U0800 F1=0.723/SAE=19.6%且FP=767。结论优先级：U0789/U0800 先做电路一致性/force-off 守卫，U842 做P0-P3泛化选型，U0778 修OFF虚块+高温低估，U2844 修6月FN/低真值FP与bus_guard滑窗锚。
- 已完成（2026-08-04 session 24）：**按 5用户 P0→P4 优先级做优化验证** → `scripts/experiment_5user_priority_remediation.py` + `FIVE_USERS_PRIORITY_REMEDIATION_VALIDATION_REPORT.md`；P4 日报监控字段已生产化进 `metrics_utils.py` 并经 5用户 force-retrain (`batch_run_20260804_070114`) 验证，daily CSV 新增 `coverage_samples_96/bus_coverage_288/branch_coverage_96/partial_day/no_positive_day/f1_lt_90_on/sae_gt_20_on`。B0 pooled inference F1=0.849/Precision=0.774/FP=1750/MAE=145.98/SAE=10.08%；P0 oracle branch-off U0789/U0800 后 F1=0.951/FP=241；P1 加 U842 focused 后 F1=0.965/FN=202/SAE=4.49%；P2 加 U0778 off guard 后 F1=0.977/FP=84；P3 加 U2844 low-true FP guard 后最终 F1=0.980/Precision=0.993/FP=46/MAE=86.03/SAE=7.09%。用户最终：U842 F1=0.979/SAE=0.74%，U2844 F1=0.918/SAE=5.77%，U0778 F1=0.996/SAE=9.23%，U0789 F1=0.998/SAE=10.10%，U0800 F1=0.978/SAE=30.79%。注意 P0/P2/P3 仍用 inference branch 标签作为诊断上限，不能直接生产；生产需 train/val-only bus/branch 一致性信号。
- 已完成（2026-08-04 session 25）：**按用户要求修改配置补充训练集外 6月样本并 5用户重训验证** → 新增 `data/time_filters_augmented_forceoff.json` + `FIVE_USERS_AUGMENTED_TRAIN_CONFIG_VALIDATION_REPORT.md`；新增训练日全部来自6月并从 inference 排除，7月仍 OOD。结果失败：pooled inference F1 0.8495→0.5836、Recall 0.9408→0.4413、FN 378→3272、SAE 10.08%→64.20%、kWh_pred 1225→382；U0789 best_thr 0.02→0.39 后 TP=0/F1=0，U0778/U0800 Recall 大幅崩。结论：补充 OFF/异常样本必要，但不能直接合入主 NILM 分类器训练，应作为独立 force-off/consistency guard 的训练样本；`data/time_filters_augmented_forceoff.json` 不建议生产使用，生产主配置仍保留 `data/time_filters.json`。
- 已完成（2026-08-07 session 26）：**独立 force-off/consistency guard 数据集 + raw bus 物理信号 + 每用户方向验证** → `scripts/experiment_5user_independent_guard.py` + `FIVE_USERS_INDEPENDENT_GUARD_VALIDATION_REPORT.md`；主模型仍用生产 `data/time_filters.json`，新增6月样本只作为 guard 数据，特征含 p_on/y_pred/区间、天气、raw load_iden、diff/rolling、day/segment raw bus 统计。结果：TV_point_guard_physical pooled F1 0.8495→0.8503、FP 1750→1730、FN 378→382、SAE 10.08%→9.62%，收益极小；day guard 无收益；U0789 仅 FP 742→722 且 FN 7→11，U0778/U0800 因 train+val predicted-ON 负样本不足禁用。结论：独立 guard 方向正确但当前信号/样本仍不足，不建议上线预测逻辑；继续只上线 P4 报表字段，后续需非空调基线/更多 OFF false-block 样本。
- 已完成（2026-08-07 session 27）：**5用户验证历史复盘 + 配置差异化归因 + 统一配置/自动生成方案** → `FIVE_USERS_VALIDATION_HISTORY_CONFIG_UNIFICATION_REPORT.md` + `scripts/build_user_data_profile.py`；结论：效果“变差”来自 oracle上限→生产约束转换、6月guard样本直接合入主分类器导致过保守、train/val负样本不足、配置混合 main/guard/calib/eval 职责。自动配置建议拆分 `global_defaults/users.<uid>.target/main_model/guard/calibration/evaluation`，生成器输出 `artifacts/auto_config_blueprint/*`；当前画像显示 U0778/U0789/U0800 main train OFF 日=0，必须独立 guard 数据集，不应污染 main_model。
- 下一会话：待指派（候选：P0 no-positive day 报表口径 / 实现 auto-config 生成器为正式配置草案 / 非空调基线估计与总线分解特征 / 补采 U0789/U0800/U0778 OFF false-block 样本 / 将 U842 P0-P3 guards 纳入 train-val-only 泛化选型 / P1 lowprob 梅雨guard 灰度配置化 / P2 低功率长时ON baseline高估修6/19&6/27 / P3 补桶样本后重估 / P-CE5 guard 滑窗锚 / P-CE6 日报 coverage 列 / 8 月数据回流）
