...
- 已完成（2026-08-03 session 12）：**P2 引入真正原始总线/段级特征验证** → `U842_P2_RAWBUS_SEGMENT_REPORT.md`；使用 raw `load_iden_data73/1/74/2/79/7/75/5/77/3/76/4` 15min值 + diff/rolling + 日级raw统计 + baseline预测ON段内raw统计，P2仍与P1分离且不改分类；test MAE 27.67→21.95W、SAE 3.62%→2.49%；inference MAE 108.07→93.22W、SAE 14.60%→7.49%、kWh_pred 192.09→208.07、SAE>20%天 14→11。风险：6/19/6/27 低功率长时ON高估加重，6/8/7/6分类整日漏检仍无效。结论：rawbus segment 是当前最佳 P2 候选，下一步需安全闸 + P1组合灰度。
- 下一会话：待指派（候选：P0 no-positive day 报表口径 / P1 U842 recall guard 配置化灰度 / P2 rawbus安全闸+P1组合验证 / P3 补桶样本后重估 / P-CE1 泛化守卫 / P-CE5 guard 滑窗锚 / P-CE6 日报 coverage 列 / 8 月数据回流）
