...
- 下一会话：待指派（候选：P0 no-positive day 报表口径 / 5用户通用电路一致性/force-off 守卫 / 将 U842 P0-P3 guards 纳入 train-val-only 泛化选型 / P1 lowprob 梅雨guard 灰度配置化 / 补采/构造低功率长时强错配样本 / 非空调基线估计与总线分解特征 / P2 低功率长时ON cap/anchor 灰度 / P3 补桶样本后重估 / P-CE5 guard 滑窗锚 / P-CE6 日报 coverage 列 / 8 月数据回流）
